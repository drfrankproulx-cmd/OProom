              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g., Mandible fracture, Malocclusion..."
                ...
                <div className="relative">
                  ...
                  <div className="relative flex">
                    <Input
                      className="h-10 text-sm rounded-l-lg rounded-r-none border-r-0 flex-1"
                      ...
                      placeholder="Search or click arrow for suggestions..."
                      data-testid="procedure-cpt-input"
                    />
                    {/* Dropdown Arrow Button */}
                    <button
                      type="button"
                      onClick={handleCptDropdownClick}
                      className="h-10 px-3 bg-gray-100 hover:bg-gray-200 border border-gray-300 border-l-0 rounded-r-lg transition-colors flex items-center justify-center"
                      data-testid="cpt-dropdown-btn"
                      title={intakeForm.diagnosis ? `Show CPT codes for: ${intakeForm.diagnosis}` : 'Show common procedures'}
                    >
                      {cptSearchLoading ? (
                        <div className="animate-spin h-4 w-4 border-2 border-teal-500 border-t-transparent rounded-full"></div>
                      ) : (
                        <svg 
                          className={`w-4 h-4 text-gray-600 transition-transform ${showCptDropdown ? 'rotate-180' : ''}`} 
                          fill="none" 
                          stroke="currentColor" 
                          viewBox="0 0 24 24"
                        >
                  ...
                  {showCptDropdown && cptSearchResults.length > 0 && (
                    <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                      {/* Show header based on category */}
                      {cptSearchResults[0]?.category && (
                        <div className={`px-3 py-1.5 border-b sticky top-0 ${
                          ...
                          <span className={`text-xs font-semibold flex items-center gap-1 ${
                            ...
                            {cptSearchResults[0]?.category === 'Favorites' ? (
                              ...
                              <>
                                ...
                                {cptSearchResults[0]?.category} Procedures
                              </>
                            )}
                            {intakeForm.diagnosis && cptSearchResults[0]?.category !== 'Favorites' && (
                              <span className="ml-1 text-xs font-normal opacity-75">
                                (for "{intakeForm.diagnosis}")
                              </span>
                            )}
                          </span>
                        </div>
